...
From the paper (Section 4.1):
    - 3 benchmarks: MBPP, HumanEval, APPS
    ...
    - Decontamination: CodeBLEU > 0.85 removed, 10-gram overlap check
    - Data split: 80/10/10 stratified by source benchmark
"""
...
@dataclass
class CodeProblem:
    """A single coding problem with metadata."""
    problem_id: str
    prompt: str
    tests: List[str]
...
def load_apps(path: str, max_problems: int = 500) -> List[CodeProblem]:
    """Load APPS dataset from directory structure."""
...
def generate_candidates_stub(
    ...
    In production, calls GPT-4o-mini with 3-shot prompting
    to generate n candidate solutions per problem.
    Each candidate is a dict with key "code".
    """
    candidates = []
    for i in range(n):
        candidates.append({
...
def verify_candidates(
    ...
    """
    Execute each candidate against unit tests.
    Classify as correct (all pass), partial (some pass), or incorrect (none pass).
    """
    verified = []
    for cand in candidates:
        pass_rate, passed, total = compute_test_pass_rate(
            cand["code"], problem.tests, timeout=timeout
        )
...
def check_ngram_contamination(
    ...
    """Return True if any n-gram overlap exists between text and test_texts."""
    text_ngrams = get_ngrams(text, n)
    for tt in test_texts:
        if text_ngrams & get_ngrams(tt, n):
            return True
    return False
...
def check_codebleu_contamination(
    ...
    """Return True if candidate is too similar to any test solution."""
    for ts in test_solutions:
        score = compute_codebleu(candidate_code, ts)
        if score > threshold:
            return True
    return False
...
def prepare_dataset(
    ...
    1. Load benchmarks (MBPP, HumanEval, APPS)
    2. Generate up to 10 candidate solutions per problem
    3. Verify candidates against unit tests (10s timeout)
    4. Decontamination: remove CodeBLEU > 0.85 and 10-gram overlaps
    5. Build training records (failing candidates as buggy solutions)
    ...
    test_solutions = [p.reference_solution for p in all_problems]
    ...
    for problem in tqdm(all_problems, desc="Building dataset records"):
        ...
        failing = [c for c in problem.candidate_solutions if c["status"] in ("incorrect", "partial")]

        for cand in failing:
            if check_codebleu_contamination(cand["code"], test_solutions):
                continue
